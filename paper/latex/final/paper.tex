...
drifting. We prove six formal properties: the regulatory actions always
...
\begin{enumerate}[leftmargin=*]
  ...
  \item We prove \textbf{six formal properties} guaranteeing predictable
...
Process calculi provide formal precursors: the Chemical Abstract
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    \midrule
    Stable & Both metrics in-band for $W = 5$ consecutive cycles \\
    Limit cycle & Metrics oscillate periodically \\
    Diverging & Structure grows without stabilizing \\
    Exhausted & Neither converges nor cycles within step limit \\
    \bottomrule
  \end{tabular}
\end{table}
...
We state six properties with proof sketches. Table~\ref{tab:properties}
summarizes them.
...
When $\mathrm{coord}(e) < 0.3$ and \textsc{bind} fires, the resulting
coordination is strictly higher.
\end{property}
\begin{proof}
...
or modify any gate. Therefore
$\mathrm{gated}(\textsc{bind}(e)) = \mathrm{gated}(e)$. The regulatory
actions operate on orthogonal dimensions.
\end{property}

\begin{property}[Termination]\label{prop:term}
The evaluator executes at most
$\mathrm{max\_iterations} \times O(\mathrm{max\_nodes} \times \mathrm{max\_depth})$
cycles. Total work is bounded.
\end{property}
...
\paragraph{Corollary 7 (Sycophant exclusion).}
A governance structure where all gates have threshold below reach
($\theta < r$ for all gates) does not converge. \textsc{bind} and
\textsc{veil} fire every cycle; both regulators are constantly active
but neither can achieve sustained stability.
...
The model, interpreter, 141 tests, formal semantics, and live
